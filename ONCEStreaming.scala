...
import scala.collection.mutable.ArrayBuffer
  * Created by pengsizhe on 2017/9/14.
  */
class Item
{
  var logo=0
...
object ONCEStreaming{
  ...
  def ValidateAndEliminate(items:ArrayBuffer[Item],constrainedTime:Int): Boolean =
  {
    ...
    if(truenum==(items.size-1)&&(isSure))
    {
      for (item<-items)
      {
        ...
        {
          // print(item.Item_Time(recordId(i)))
  ...
  var Empty_Item=0
  // var a= init(2, 68, 65, 44, 22, 12, 44, 22, 8)
  // var c=init(2, 68, 65, 44, 59)
  // var b=init(2, 68, 65, 44, 22, 12, 44)
  // var d=init(8,8,9)
  var e=init(8,8)
  def work(Seq:List[(Int,Int)],begin:Int,block:Int):ArrayBuffer[Int]=
  {
    var freq_S= countseq_S(Seq,begin,Seq.size,e)
    /*var freq_C=countseq_C(Seq,begin,block,e)
    if(freq_C(1)!=(-1))
    {
      println("最后符合条件的位置是"+freq_C(1))
    }
    */
    // countseq_S(Seq,b)
    // countseq_S(Seq,c)
